...
**Актуальность темы.** При мелкосерийном или штучном изготовлении электронного оборудования одним из этапов является изготовление печатной платы - изделия, состоящее из плоского изоляционного основания с отверстиями, пазами, вырезами и системой проводников, которое используют для установки компонентов в соответствии с электрической принципиальной схемой. Достаточно часто на этом этапе заготовку печатной платы изготавливают методом фрезеровки и сверления на CNC станке. CNC-станок для создания печатных плат — это станок, оснащенный  рабочим столом, на котором крепится заготовка из фольгированного стеклотекстолита. Фрезерование печатных плат по сути ничем не отличается от других операций, выполняемых на станках с числовым программным управлением. Преимущество этого метода заключается в том, что он позволяет выполнять обработку с высокой степенью точности при минимальном участии человека в процессе. Процесс изготовления печатных на CNC станке начинается с разработки схемы платы в специализированой программе (Sprint Layout, Fritzing и др.). Схема печатной платы экспортируется из программы разработки в Gerber формате, после чего необходимы вычислить траекторию обрабатывающего инструмента (G - код файл) программой Visolate [40] или другой. Когда программа управления будет готова, необходимо выполнить инициализацию станка и прочно закрепить заготовку печатной платы на рабочем столе фрезера, особое внимание уделить параллельности краев заготовки строительным осям станка. Вручную выполнить "поиск 0" - совместить острие инструмента с левым нижним углом заготовки. Запустить программу управления и дождаться её оканчания.

В статье [1] были рассмотрены основные компоненты, достоинства и недостатки станков с ЧПУ, а также приведен пример разработки станка с подъемным рабочим столом. В работе [2] представлен обзор систем управления фрезерным оборудованием. Авторы статьи [3] детально разобрали системы управления ЧПУ станков и их переход на новую элементарную базу. Ученые из Сибирского государственного аэрокосмического университета им. академика М.Ф. Решетникова [4] подчеркивают важность практического ознакомления с изготовлением печатных плат в учебном процессе, результаты настоящего ВКР в будущем будут использоваться в центре коллективного пользования ФГАОУ ВО "ПНИПУ". В статье [5] приведен обзор современного CNC станка для изготовления печатных плат фирмы LPKF [6]. Исследователи из Орхусского университета [7] разрабатывают систему ЧПУ для лазерного резака, основанную на подходе WYSIWYG, где проектор используется для отображения текущих контуров, а маркеры используются для установки его положения в рабочей области. Кроме того, специалисты университета Кейо [8] расширяют функциональность фидуциальных маркеров для лазерного резака, размещая набор маркеров опорных точек рядом с заготовкой. В работе [9] описан способ обнаружения ошибок контура на основе машинного зрения с использованием специального измерительного прибора с нанесенными на него маркерами. В работе [10] предложен метод прямой симуляции обрабатывающего инструмента станка с использованием маркеров для определения положения инструмента и заготовки, а дополненная реальность используется для моделирования траектории инструмента при обработке.

Различные системы управления CNC-станками для изготовления печатной платы включают в себя: Mach3 [11], LinuxCNC [12], Bantam Tools [13], GRBL Controller [14], BoardMaster [15]  и др.. Описанные ранее системы имеют примерно одинаковый функционал: ручное управление, отправка ранее подготовленных G-код [16] программ, визуализация G-код [16] программ. Но при этом не решаются проблемы идентификации заготовки печатной платы на рабочей области фрезера, фотографическая визуализация возможного результата, фотографический контроль результата работы оборудования, автоматическое определение смещения координат обработки при фрезеровки детали с оборотной стороны, генерация команд для станка по рисунку схемы печатной платы.
...
С учетом поставленной задачи и имеющихся средств была разработана Функциональная схема [17] системы управления программно-аппратной системы управления cnc станка (Рисунок 2).
...
Схемотехническое проектирование аппаратной части  программно-аппартаной системы управления CNC станка заключается в разработке принципиальной схемы [17] устройства (рисунок 3).
...
Каркас станка был взаимствован от пройстешего китайского CNC станка. Для дальнейшей сборки аппаратной части станка необходимо было смоделировать ряд деталей и корпусов. При помощи среды моделирования Blender [18] подготовлена модель крышки для камеры показанная на рисунке 4. 
...
При помощи среды моделирования Blender [18] подготовлена модель заднего левого правого разъема основного корпуса показанная на рисунке 11. 
...
При помощи среды моделирования Blender [18] подготовлена модель левыого и правого разъемов крышки основного корпуса  показанная на рисунке 13. 
...
При помощи среды моделирования Blender [18] подготовлена модель основного корпуса, подставки для raspberry pi [19] показанная на рисунке 16. 
...
При помощи среды моделирования Blender [18] подготовлена модель держателя блока питания показанная на рисунке 21. 
...
В этой главе было подготовленно и согласовано с руководителем (Приложение A) техническое задание [21] к проекту, согласно которому была разработана функциональная схема [17], принципиальная схема [17], разработанны 3-д моделли конструкции станка в среде моделирования Blender [18], модели подготовленны к печати и напечатаны на 3-д принтере программным комплексом cura [20], а также собранное устройство протестированно на предмет верного подключения и сборке компонентов.
...
В начале происходит выбор способа нумерации пинов GPIO [23] методом setmode() модуля Rpi.GPIO [], был выбран метод нумерациии BOARD, так как он обеспечивает работоспособность оборудования независимо от версии микрокомпьютера Raspberyy Pi [19]. Далее методом setwarnings() были включенны предупреждения, на случай если какой-то контакт был настроенн и используется другой задачей в системе. После, настраиваются пины концевых датчиков, управления драйверами шаговых двигателей и настраивается ШИМ [24] управления навесным оборудованием.
...
Методу get_frames() модуля OpenCv [26] передаётся id номер камеры, далее создаётся экземпляр класса cv.VideoCapture() модуля OpenCv [26] с нужным номером камеры и проверяется исправность её открытия. После чего настраиваются бозовые свойства камеры и из четырёх последовательно полученных с камеры изображений пользователю отдаётся кадр повышенного в два раза, чем стандартное, разрешения [30].
...
Для исправления перспективы съемка велась таким образом, чтобы рабочий стол помещался в поле зрения камеры с достаточными "полями", затем в графическом редакторе были получены фактические  координаты четырёх углов стола и требуемые координаты для исправления перспективы, из полученных данных строилась матрица преобразования при помощи функции cv2.getPerspectiveTransform (src, dst) [26]. При её применении к изображению, а также пропустив изображение через функцию cv2.warpPerspective (src, M, dsize) [26] мы получаем  изображение в нужной проекции. Остаётся только обрезать изображение по контуру рабочего стола и передать его следующим функциям системы. 
...
В работе [27] рассмотрено решение подобной задачи, но в условиях нашей задачи мы можем упростить работу, так как мы знаем размер заготовки (заготовки печатных плат имеют известные параметры ширины, выстоты и толщины) [28]. Применив функцию cv2.findContours() [26], мы получим список всех найденных на изображении контуров. Посчитав диагональ, меньшую и большую стороны каждого контура и зная диагональ, меньшую и большую стороны искомого, выбираем нужный контур и передаём координаты углов контура следующим функциям системы (рис. 30).
...
Сперва загружаем, полученный на предыдущем шаге, файл и применяем функцию cv.cvtColor() [26] для перевода изображения в градации серого, потом используем функцию cv.threshold() [26] для получения двоичного изображения, которое преобразовывается согласно пороговому значению. Функцией cv.findContours() [26] получаем все контуры дорожек печатных плат и  рисуем все эти контуры тем же цветом, что и дорожки печатной платы. Далее снова находим контуры на изображении, но в этот раз закрашиваем голубым цветом. Результат поиска контуров представлен на рисунке 32.
...
Для оценки качества позиционирования, возьмём два изображения, первое возьмём за Исходное, а второе за Тестируемое. Загрузим оба изображения в программу методом cv.imread() [26] и преобразуем цветовую палитру в градиции серого аргументом cv.IMREAD_GRAYSCALE [26], после вызовем к обоим изображениям метод cv.threshold(), для получения порогового определения изображения с применением бинаризации Оцу [31], предназначенного для автоматического определения порога изображения, далее вычтем из Тестируемого изображения Исходное и исследуем результат, код этой программы представлен на Листинге 13.
...
3) Было выполненно схемотехническое проектирование аппаратной части  программно-аппартаной системы управления CNC станка;

4) Была составлена спецификация аппаратной части программно-аппартаной системы управления CNC станка;

5) Была собранна и протестированна аппаратная часть  системы управления CNC станка;

6) Были разработанны алгоритмы управления программно-аппартаной системы управления CNC станка;

7) Было написанно программное обеспечение программно-аппартаной системы управления CNC станка;

8) Было произведено тестирование программно-аппартаной системы управления CNC станка.

В результате была полученна интеллектуальная система управления процессом изготовления печатных плат для CNC станка.

Данный отчет, отражающий основные результаты в ходе выполнения выпускной квалификационной работы, выполнен в соответствии с ГОСТ 7.32-2001 [38]. Структура и состав представленного отчета соответствует требованиям, предъявляемым к документации по магистерским диссертациям кафедры «Информационные технологии и автоматизированные системы» Пермского национального исследовательского политехнического университета. 

Созданная система была отмеченна на 26-м Московсом международном Салоне изобретений и инновационных технологий "Архимед 2023" [39] и решением международного жюри был награждён бронзовой медалью (Приложение В).
...
1. Дожделев А.М., Лаврентьев А.Ю., Какорин Д.Д., Туляев И.П. КОМПОНОВКА ФРЕЗЕРНЫХ СТАНКОВ С ЧПУ // Международный журнал гуманитарных и естественных наук. 2022. №12-2. URL: https://cyberleninka.ru/article/n/komponovka-frezernyh-stankov-s-chpu (дата обращения: 12.06.2023).

2. Сокур А. А. Система управления фрезерным станком по изготовлению печатных плат // МНИЖ. 2013. №5-1 (12). URL: https://cyberleninka.ru/article/n/istema-upravleniya-frezernym-stankom-po-izgotovleniyu-pechatnyh-plat (дата обращения: 12.06.2023).

3. Белоусов Е. Ф., Литвинский В. С. Станок для изготовления печатных плат // НиКа. 2009. №. URL: https://cyberleninka.ru/article/n/stanok-dlya-izgotovleniya-pechatnyh-plat (дата обращения: 12.06.2023).

4. Лебедев А.В., Гринберг Г.М. Лабораторная работа «Изготовление печатных плат методом оконтуривания» // Актуальные проблемы авиации и космонавтики. 2011. №7. URL: https://cyberleninka.ru/article/n/laboratornaya-rabota-izgotovlenie-pechatnyh-plat-metodom-okonturivaniya (дата обращения: 05.06.2023).

5. Петров Михаил Оборудование фирмы LPKF: производство печатных плат в домашних условиях // Компоненты и Технологии. 2002. №24. URL: https://cyberleninka.ru/article/n/oborudovanie-firmy-lpkf-proizvodstvo-pechatnyh-plat-v-domashnih-usloviyah (дата обращения: 05.06.2023).

6. LPKF // lpkf.com URL: https://www.lpkf.com (дата обращения: 12.06.2023).

7. Winge K., Haugaard R., and Merritt T. Val: Visually augmented laser cutting to enhance and support creativity //
...
8. Kikuchi T., Hiroi Y., Smith R., Thomas B., and Sugimoto M. Marcut: Marker-based laser cutting for personal
...
9. Li X., Liu W., Pan Y., Li H., Ma X., and Jia Z. A monocular-vision based contouring error detection method for CNC
machine tools // IEEE Intern. Instrumentation and Measurement Technology Conf. (I2MTC). 2018. May. P. 1—6.

10. Kiswanto G. and Ariansyah D. Development of augmented reality (ar) for machining simulation of 3-axis cnc
milling // Intern. Conf. on Advanced Computer Science and Information Systems (ICACSIS). 2013. September.
P. 143—148.

11. Система управления Mach3 // ntma.com.ua URL: https://ntma.com.ua/ru/mach3.html (дата обращения: 12.06.2023).

12. Система управления LinuxCNC // wiki.linuxcnc.org URL: http://wiki.linuxcnc.org/cgi-bin/wiki.pl?LinuxCNCVersion2 (дата обращения: 12.06.2023).

13. Система управления bantamtools // bantamtools.com URL: https://www.bantamtools.com/ (дата обращения: 12.06.2023).

14. Система управления GRBL // github.com URL: https://github.com/orgs/grbl/repositories (дата обращения: 12.06.2023).

15. Система управления boardmaster // boardmaster.ru URL: https://www.boardmaster.ru/ (дата обращения: 12.06.2023).

16. Устройства числового программного управления для металлообрабатывающего оборудования. Кодирование информации управляющих программ [Текст] : ГОСТ 20999-83.

17. Правила выполнения электрических схем [Текст] : ГОСТ 2.702-2011 ЕСКД.

18. Blender // blender.org URL : https://www.blender.org/ (дата обращения: 12.06.2023).

19. Raspberry Pi Documentation // www.raspberrypi.com URL: https://www.raspberrypi.com/documentation/ (дата обращения: 16.5.2023).

20. UltiMaker Cura // ultimaker.com URL : https://ultimaker.com/software/ultimaker-cura/ (дата обращения: 12.06.2023).

21. Комплекс стандартов на автоматизированные системы. Техническое задание на создание автоматизированной системы [Текст] : ГОСТ 34.602-2020. 

22. Bitmap // fileformats.archiveteam.org URL: http://fileformats.archiveteam.org/wiki/BMP (дата обращения: 12.06.2023).

23. RPi.GPIO  // sourceforge.net URL: https://sourceforge.net/projects/raspberry-gpio-python/ (дата обращения: 16.5.2023).

24. Мяхор Дмитрий Александрович Разработка аппаратной составляющей программно-аппаратного образовательного робототехнического комплекса // Интерэкспо Гео-Сибирь. 2018. №8. URL: https://cyberleninka.ru/article/n/razrabotka-apparatnoy-sostavlyayuschey-programmno-apparatnogo-obrazovatelnogo-robototehnicheskogo-kompleksa (дата обращения: 12.06.2023).

25. Драйвер шагового двигателя A4988 // tme.eu URL: https://www.tme.eu/Document/25459777e672c305e474897eef284f74/POLOLU-2128.pdf (дата обращения: 16.5.2023).

26. OpenCV: OpenCV-Python Tutorials // docs.opencv.org URL: https://docs.opencv.org/4.x/d6/d00/tutorial_py_root.html (дата обращения: 16.5.2023).

27. Алгоритм нахождения контура делового остатка нестандартной формы по цифровой фотографии средствами языка программирования Python на основе библиотеки компьютерного зрения Opencv / В. Р. Зайникова, С. А. Зыкин, Р. А. Файзрахманов // Вестник Пермского национального исследовательского политехнического университета. Электротехника, информационные технологии, системы управления = Perm National Research Polytechnic University Bulletin. Electrotechnics, Information Technologies, Controlsystems. - 2020. - № 35. - С. 133-151.

28. Макетные платы и стеклотекстолит | купить в розницу и оптом // www.chipdip.ru URL: https://www.chipdip.ru/catalog/breadboards (дата обращения: 16.5.2023).

29. Canny J.E. A computational approach to edge detection // IEEE Trans Pattern Analysis and Machine Intelligence. – 1986. – No. 8. – Р. 679–698.

30. Попов М. А., Станкевич С. А., Шкляр С. В. Алгоритм повышения разрешения субпиксельно смещенных изображений // ММС. 2015. №1. URL: https://cyberleninka.ru/article/n/algoritm-povysheniya-razresheniya-subpikselno-smeschennyh-izobrazheniy (дата обращения: 12.06.2023).

31. Исрафилов Хидирнеби Сираждинович Исследование методов бинаризации изображений // Вестник науки и образования. 2017. №6 (30). URL: https://cyberleninka.ru/article/n/issledovanie-metodov-binarizatsii-izobrazheniy (дата обращения: 12.06.2023).

32. Our Documentation | Python.org // www.python.org URL: https://www.python.org/doc/ (дата обращения: 16.5.2023).

33. Numpy Documentation // numpy.org URL : https://numpy.org/doc/ (дата обращения: 12.06.2023).

34. Scipy Documentation // docs.scipy.org URL : https://docs.scipy.org/doc/scipy/ (дата обращения: 12.06.2023).

35. Math Documentation // docs.python.org URL : https://docs.python.org/3/library/math.html (дата обращения: 12.06.2023).

36. Statistics Documentation // docs.python.org URL : https://docs.python.org/3/library/statistics.html (дата обращения: 12.06.2023).

37. Репозиторий с исходным кодом проекта // github.com URL : https://github.com/Honepa/pyCNC/tree/master (дата обращения: 12.06.2023).

38. Отчет о научно-исследовательской работе. Структура и правила оформления [Текст] : ГОСТ 7.32-2001.

39. Салон "Архимед 2023" // салон-архимед.рф URL : http://салон-архимед.рф/ (дата обращения: 12.06.2023).

40. Visolate // github.com URL : https://github.com/Traumflug/Visolate (дата обращения: 12.06.2023).
